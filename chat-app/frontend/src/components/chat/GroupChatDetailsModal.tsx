...
const GroupChatDetailsModal: React.FC<{
  ...
  createNewChat:(selectedUserId: string) => Promise<any>;

}> = ({ open, onClose, chatId, onGroupDelete,createNewChat }) => {
  const { user } = useAuth();
...
const [loading,setLoading] = useState(false)
  ...
  //   );
  // };
  // const createNewChat = async (fromGroup:any) => {
  //   // If no user is selected, show an alert

  //   // Handle the request to create a chat
  //   await requestHandler(
  //     // Callback to create a user chat
  //     async () => await createUserChat(fromGroup),
  //     setLoading, // Callback to handle loading state
  //     // Success callback
  //     (res) => {
  ...
  //         return;
  //       }
  //       // getChats() // Execute the onSuccess function with received data
                            ...
                            {groupDetails?.participants?.map((part) => {
                              return (
                                <React.Fragment key={part._id}>
                                  <div className="flex justify-between items-center w-full py-4" onClick={()=>createNewChat(part._id)}>
